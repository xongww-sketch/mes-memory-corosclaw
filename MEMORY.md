...
## ⛔ 硬规则：文件交付方式（熊旺 2026-08-12 再次强调）

**交付任何文件（SQL / JSON / Excel / MD / 报告），一律用 `message` 工具带 `media` 参数直接发飞书文件附件。**
- ❌ 绝不用 `MEDIA:路径` 这种链接/路径形式
- ❌ 绝不只在正文里写文件路径让对方自己找
- ✅ 必须是飞书里能直接点开下载的文件本体
- 可以在消息里附一句简短说明，但**文件本体必须作为附件发送**
- 2026-08-11 已确认过一次，2026-08-12 因再犯被熊旺纠正（「请你以后发文件记住，以飞书的形式直接发文件给我，不要给链接，请记谨记」）
...
### 📈 品质视图 V3 统一口径改造（2026-08-11 启动）

**发起人**：熊旺
**需求文档**：https://coros-teams.feishu.cn/docx/SoLXdNve7orhpsxqNRzcFNHQnnd
**现有工具**：不良品视图 lowcode（applicationId: 6a7199178f6b911...）

#### 核心决策（2026-08-11）

**1. 口径统一（压倒性优先级）**
- 熊旺要求：「所有数据查询逻辑统一，不要一边高一边低」
- 7个品质工作流全部改造，不仅限 Tab2/Tab5
- 失效时点统一用 `defective_date`（不再用 duration_days 反算）
- 计数方式统一用 `COUNT(DISTINCT ar.retroid)`
- 脏数据过滤统一：排除 defective_date 为空(6条)、未来日期(1条)、不良早于激活(239条)，共246条

**2. 算法一行不动**
- 威布尔拟合、失真分界、失效数<5不出曲线等算法逻辑完全保留
- 只更换取数来源和筛选条件

**3. 需求范围（9条）**
- Tab5：激活时间多选(5窗口) + 机型多选(5窗口) + 一级标签(自动全纳入)
- Tab2：不良日期筛选 + 型号/日期多选(5窗口)
- 全局：性能优化（B18查不出）+ 命名优化
- Tab6：多档截止时长 → **本轮不做**

**4. 一级标签语义**
- 选中一级分类 → 后端自动展开其下全部二级计入统计
- 前端只传 `primary_labels`，二级下拉保留供精细筛选
- 故障类型数据：850条组合值 → 94个原子标签，89%为多故障组合
- 拆分铁律：按 `,` `，` `;` `；` 拆；括号内不拆；顿号不拆

**5. 命名定案**
- 「真实年月（按日历时间）」→ **「累计不良率（按故障发生月）」**
- 「使用月龄」补全为 **「累计不良率（按使用月龄）」**
- 理由：该模式真正特征是分母滚动增长，原名完全没体现

#### 关键技术发现

**defective_date 可靠性验证**（56,933条全表）：
- 93.19% 与反算时点偏差≤1天
- 3,843条 duration_days 为负四万多天（Unix时间戳溢出），反算失效时点掉到1901-1905年
- **现网缺陷**：Tab5/Tab6 含 `AND dr.defective_duration_days >= 0`，把3,843台真实故障设备全部挡在统计外 → 不良率偏低约6.75%

**影响面实测**（熊旺提供SQL结果）：
- Tab5/6 新增计入3,154台（+5.95%）
- Tab1234 减少718台（-1.26%）
- 主力机型最大变化：B20 +0.48pp，B19S +0.29pp，W331 +0.17pp
- **关键判断**：最大仅0.48pp，无量级跳变 → 一次改完，不拆两步

**计划外发现**：
- `activation_records` 有9,230条重复行（0.33%），新口径顺带修掉
- 三个机型疑似售后数据断链：M1D（11万激活仅1条不良）、B15D、FD01 → 建议单独立项

#### 实施状态（2026-08-11 晚开工）

**工作目录**：`quality-forecast/v3-unified/`
- 基线副本：`v3-unified/workflows/baseline/`
- 核心文档：`v3-unified/docs/统一口径规范.md`（唯一事实来源）
...
**基线复核**：n8n API实时拉取7个工作流，SHA256比对 7/7 全部一致 ✅

**状态**：4个子任务并行执行中，生产环境自始至终未改动

---

### 📈 品质视图 V4 改版（2026-08-12 全天交付）

**需求基线**：陈明 8/7 群内文字清单 7 条（无第4条），会议纪要仅参考

**核心变更**（相对V3）：
- 推倒重来，以陈明清单为唯一基线
- 机型改单选（1组=1机型=1曲线），解决同名曲线冲突
- 故障类型改为多级级联（treeSelect，非cascader——cascader不支持多选）
- 默认2组可加到5组，Tab2/Tab5同样
- 发现并修复长期漏统计缺陷：`defective_type IN ('GPS异常')`精确匹配命中不了90.69%的组合值

**defective_type 数据结论**（56,933条）：
- 14个一级标签，93个二级进下拉
- 分隔符规则：半角逗号/分号✅拆；**顿号❌绝不拆**（`按键、旋钮异常`是完整标签）；括号必须保护
- 匹配方案：后端展开一级→原子标签→LIKE边界条件，1次查询

**性能优化核心发现**：
- Tab1真凶：`COALESCE(从表列, 主表列)` 导致索引完全失效→271万行全表扫
- 教训：判断SQL能否走索引，不能只看「列是否被函数包裹」，还要看「条件是否跨表」
- Tab3：flatMap展开→222次往返，改为VALUES表JOIN→2次
- 数值对拍抓到的两次致命错误（只验结构会出事）

**Tab与工作流对应（易混，务必记住）**：
- Tab1 市场故障率趋势 → 工作流名 `DefectiveCountry_API`，webhook path `/prod/DefectiveOverall_API`
- Tab3 → `Defectivedetails_API`
- Tab4 → `Defectiverange_API`
- Tab6 `Forecast_API` 指向 `/prod/DefectiveMonthlyTrend_API_TEST`（历史命名，非测试环境）

**文件位置**：`quality-forecast/v4/`
...
- query triggerType：仅`automatic`/`manual`
- treeSelect：`selectTypeOptions=single|multi|check`；`checkedStrategyOptions=all|parent|child`；父子联动靠`checkStrictly:false`
- cascader不支持多选，多选级联必须用treeSelect
- tempState.value是字符串，运算必须Number()强转
- dateRange空值返回`"1970-01-01"`而非空串，前后端都要归一化
...
**状态**：深度梳理中。Jackson 持续 SAP 知识补课：
- 07-16：主数据体系、条件技术、转让定价、公司股权架构、越南/日本代工逻辑
- 08-06：组织架构三层工具（公司代码/利润中心/COPA特征）、跨公司转储代价、分销渠道 vs COPA渠道区别
- **关键认知对齐**：MES 的 channelId → SAP 侧是 COPA 特征（分析标签），不是利润中心也不是分销渠道
- **⚠️ 工作 convention（08-07 熊旺要求）**：解释所有 SAP/MES 概念时必须结合"为什么要这么做"的设计原理，不能只说"是什么"
- 尚未开始落地画图，待熊旺确认累计的业务逻辑后重新画图。
