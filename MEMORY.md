...
### 🍎 Apple FindMy 产线对接方案（2026-07-13 启动）

**发起人**：熊旺
**目标**：设计长效稳定的 FindMy Token 注册/激活/校验/下发方案，覆盖所有维修场景。

**核心需求**：
1. 密钥序列号唯一性校验 + token 长度/合法性校验
2. 固件写入后立即通过 Apple 接口反查写入成功
3. 维修场景全覆盖（主板复用、token 替换、主板换到新 SN 底壳）
4. 激活状态可追踪（利于产线校验）
5. 长效稳定、一次性解决

**关键细节**：主板通过 UUID 区分；FindMy Token 通过硬件 NPI 从 MES 拉取；本地只有 token 状态机，激活状态需查 Apple 官方文档。

**参考文档**：《Find My Network Accessory Protocol Specification R3》

**状态**：需求已梳理，方案设计中。待熊旺确认激活状态查询 API 可用性及 token-UUID 绑定持久化方案。
...
**状态**：深度梳理中，尚未开始落地画图。待熊旺确认累计的业务逻辑后重新画图。
